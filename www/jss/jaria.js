...
var Jaria = {
		...
		message : [																			//Messages pouvant être redéfinis
			"L'élément HTML %1 n'existe pas !",												//0
			"Préchargement des images : %1",												//1
			"Impossible de charger l'image %1 !",											//2
			"Information",																	//3
			"Confirmation",																	//4
			"Oui",																			//5
			"Non",																			//6
			"Alerte",																		//7
			"Erreur interne!",																//8
			"Le texte html de boite de dialogue est obligatoire!",							//9
			"Déplacer",																		//10
			"Fermer [touche Echape]",														//11
			"Ok",																			//12
			"Redimensionner",																//13
			"Le délai d'attente maximum de réponse du serveur évalué à %1 s. est dépassé",	//14
			"Veuillez réessayer",															//15
			"Délai dépassé!"																//16
		],
        ...
        nav: new function(){
            ...
            $.version = ($.agent.match( /.+(?:firefox|version|pera|opr|chrome|onqueror|msie|rv)[\/: ]([\d.]+)/ ) || [])[1];
            $.msie = false;
            $.oldmsie = false;
            $.firefox = false;
            $.opera = false;
            ...
            $.showload = false;									// Afficher le préchargement des images Jaria
            $.timer = null;
            

            //Type et nom du navigateur
            $.type = function(){
                var t = Jaria.txt;
                var a = $.agent;
                 if(t.found(a, "opera") || t.found(a, "opr")){
                        $.opera = true;
                        $.name = "opera";
                }                        
                else if(t.found(a, "chrome")){
                        $.chrome = true;
                        $.name = "chrome";
                }
                else if(t.found(a, "safari")){
                        $.safari = true;
                        $.name = "safari";
                }
                else if(t.found(a, "firefox")){
                        $.firefox = true;
                        $.name = "firefox";
                }
                else if(t.found(a, "konqueror")){
                        $.konqueror = true;
                        $.name = "konqueror";
                }
                else if(t.found(a, "msie") || (t.found(a, "trident") && t.found(a, "rv"))){
                        $.msie = true;
                        $.name = "msie";
                        if($.version < 9){
                                $.oldmsie = true;
                ...
                else if(t.found(a, "dolfin")){
                        $.dolfin = true;
                        $.name = "dolfin";
                }
                else if(t.found(a, "gecko")){
                        $.gecko = true;
                        $.name = "gecko";
                }
                else if(t.found(a, "@") || t.found(a, "www") || t.found(a, "http:") || t.found(a, "ask")){
                        $.robot = true;
                        $.name = "robot";
                }
                else{
            ...
            $.scroll = function(){
                ...
                $.scrollX = ( $.msie || ($.opera && $.version < 13) ) ? document.documentElement.scrollLeft : window.scrollX;
                $.scrollY = ( $.msie || ($.opera && $.version < 13) ) ? document.documentElement.scrollTop : window.scrollY;
            ...
            //Préchargement des images externes
        ...
        el: new function(){
            ...
            $.get = function(e){
                if(!$.test(e)){
                	Jaria.box.error("<b class=jaria_error>" + Jaria.txt.make(Jaria.message[0], (typeof(e) == "string") ? e : typeof(e)) + "</b>");
                    return undefined;
                }
            ...
            // supprime une class à l'élément
            $.delclass = function(e, s){
                /*
                    argument 0 : obligatoire                élément
        ...
        Loadimage: function(){
            ...
            $.start = function(o){
                ...
                var i = last;
                if(nav.ready && url.length > 0 && i < url.length){
                    if( !Jaria.el.isobject(url[i]) ){
                        var u = txt.trim(url[i]);                                        
                        url[i] = new Image();
                        url[i].src = u;
                        $.show(Jaria.txt.make(Jaria.message[1], (i + 1).toString() + " / " + url.length));
                        url[i].onerror = function(){
                            $.show("<b class=jaria_error>" + Jaria.txt.make(Jaria.message[2], u) + "</b>");
                            i ++;
                            last = i;
                        };
                    }                                        
        ...
        Box: function(){
                ...
                $.ico = true;                                       // plus utilisé depuis le 05/11/2011
                $.bts = true;                                       // affiche le(s) bouton(s)
                $.quit = true;                                      // affiche le bouton quitter dans la barre de titre de la box
                $.title = "";                                       // tire dans la barre de titre de la box
                $.html = "";                                        // html du message de la box
                $.focus = true;                                     // focus sur le bouton de la box
                ...
                $.info = function(s, t, w){
                    $.show({
                        type: 1,
                        html: s,
                        title: ((Jaria.txt.test(t)) ? t : Jaria.message[3]),
                        width: w,
                        borderColor: "#06b68f",
                        bts: false
                    });
                ...
                $.confirm = function(s, t, w){
                    $.show({
                        type: 2,
                        html: s,
                        title: ((Jaria.txt.test(t)) ? t : Jaria.message[4]),
                        width: w,
                        borderColor: "#0661b6"
                    });
                    ...
                    $.el.BtOk.html(Jaria.message[5]);
                    $.el.BtNo.className = "jaria_button";
                    $.el.BtNo.style.width = "50px";
                    $.el.BtNo.html(Jaria.message[6]);
                ...
                $.alert = function(s, t, w){
                    $.show({
                        type: 3,
                        html: s,
                        title: ((aria.txt.test(t)) ? t : Jaria.message[7]),
                        width: w,
                        borderColor: "#c6b01e",
                        bts: false
                    });
                ...
                $.error = function(s, t, w){
                    $.show({
                        type: 3,
                        html: s,
                        title: ((Jaria.txt.test(t)) ? t : Jaria.message[8]),
                        width: w,
                        borderColor: "#ea5247",
                        bts: false
                    });
                ...
                $.show = function(o){
                    ...
                    $.el.Title.title = Jaria.message[10];
                    ...
                    $.el.Head.appendChild($.el.Title);
                        ...
                        if($.bts){
                            ...
                            $.el.BtOk.innerHTML = Jaria.message[12];
                        ...
                        if($.status){
                            ...
                            $.el.status.innerHTML = "&nbsp;";
                            $.el.status.title = Jaria.message[13];
                ...
                $.resizestart = function($){
                    ...
                    function move(){                                
                        try{
                            var h = parseInt($.el.Html.startY) + parseInt(nav.mouseY) - nav.scrollY - $.el.posStartY;
                            if(!isNaN(h) && h >= 40){
                                $.el.Html.style.height = txt.px(h);
                                $.el.style.height = txt.px($.el.Head.offsetHeight + $.el.Html.offsetHeight + $.el.Bts.offsetHeight + $.el.status.offsetHeight + 3);
                            }
                            var w = parseInt($.el.Html.startX) + parseInt(nav.mouseX) - nav.scrollX - $.el.posStartX;
                            if(!isNaN(w) && w >= $.width) {
                                $.el.Html.style.width = txt.px(w);
                    ...
                    if( $.el.posStartY == null ){
                        $.el.posStartY = parseInt(nav.mouseY) - nav.scrollY;
                        $.el.posStartX = parseInt(nav.mouseX) - nav.scrollX;
                        console.log(nav.mouseY + " " + nav.scrollY);
                    }
        ...
        Ajax: function(){
                ...
                $.ontimeout = function(){
                    box.hide();
                    box.alert(Jaria.txt.make(Jaria.message[14], ($.timeout / 1000).toString()) + "<br /><br />" + Jaria.message[15] , Jaria.message[16]); 
